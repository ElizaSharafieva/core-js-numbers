...
function toNumber(value, def) {
  return Number.isNaN(Number(value, def)) ? def : Number(value, def);
}
...
function getSumOfDigits(num) {
  ...
  const sum = arr.reduce((previousValue, item) => {
    return Number(previousValue) + Number(item);
  });
  return sum;
}
...
function getCountOfOddNumbers(number) {
  ...
  return num % 2 === 0 ? num / 2 : Math.trunc(num / 2) + 1;
}
